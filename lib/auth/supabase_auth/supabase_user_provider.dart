...
class NirbySupabaseUser extends BaseAuthUser {
  NirbySupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> nirbySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = NirbySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
